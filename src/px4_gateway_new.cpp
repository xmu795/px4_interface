...
void PX4Gateway::setTarget(const px4GatewayTypes::setpoint &target) {
  ...
  traj_setpoint_msg.x = target.x;
  traj_setpoint_msg.y = target.y;
  traj_setpoint_msg.z = target.z;
  traj_setpoint_msg.vx = target.vx;
  traj_setpoint_msg.vy = target.vy;
  traj_setpoint_msg.vz = target.vz;
